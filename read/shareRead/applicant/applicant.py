...
def applicate(request):
    ...
    if applicantForm.is_valid():
        ...
        if studentEntry.status == 0:
            ...
            return render(request, 'shareRead/applicate.html',
                          {'hint': '对不起，已经有其他热心朋友选定了该小朋友，请您选择其他愿意帮助的小朋友。', 'studentEntry': studentEntry})
...
def detail(request):
    ...
    try:
        ...
        if not applicantList:
            return HttpResponse(json.dumps({"code": 0, "msg": "", 'count': count, 'data': []}),
        ...
        return HttpResponse(json.dumps({"code": 0, "msg": "", 'count': count, 'data': Rows}),
                            content_type="application/json")
    except:
        pass
